...
# ADD SCATTER PLOT
x = df_mean2['year']      # convert to list for regression calculations later
y = df_mean2['energy']
...
fig.update_layout(
    ...
    title = {
            'text': "Average energy level of Billboard Year End Hot 100 songs (1980-2019)",
            'y':0.9,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'top'
    ...
    xaxis_title = "Year",
    yaxis_title = "Energy (min=0, max=1)",
    # font=dict(
...
py.iplot(fig, filename='spotifyData_energyOverTime_2LRs')
fig.show()
